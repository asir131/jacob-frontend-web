...
import { Calendar as CalendarIcon, UploadCloud, MapPin, Send, ArrowRight, Sparkles, CheckCircle2, DollarSign, Info } from 'lucide-react';
...
export default function PostRequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!user) {
      toast.error('You must be signed in as a client to post a request.');
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
           ...
           <p className="text-slate-500 text-lg md:text-xl max-w-2xl mx-auto font-medium leading-relaxed">
             Can't find exactly what you're looking for? Tell us your needs, and we'll connect you with top local experts.
           </p>
        </motion.div>
                ...
                <CardContent className="p-8 md:p-12">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      ...
                      <div className="space-y-3">
                         <label className="text-sm font-bold text-slate-800 uppercase tracking-wider">Service Category</label>
                         <Select required>
                          <SelectTrigger className="w-full h-14 rounded-xl border-slate-200 focus:ring-[#2286BE] bg-slate-50/50 font-medium">
                      ...
                      {/* Location */}
                      <div className="space-y-3">
                         <label className="text-sm font-bold text-slate-800 uppercase tracking-wider">Service Location</label>
                         <div className="relative group">
                            ...
                            <Input type="text" placeholder="Your full address of neighborhood" required className="h-14 pl-12 rounded-xl border-slate-200 focus-visible:ring-[#2286BE] bg-slate-50/50 font-medium" />
                         </div>
                      </div>

                      {/* Description */}
                      <div className="md:col-span-2 space-y-3">
                         <label className="text-sm font-bold text-slate-800 uppercase tracking-wider">Detailed Description</label>
                         <textarea 
                           rows={5} 
                           required
                           placeholder="Describe your requirements in detail. For example: 'I need a complete deep cleaning for my 3-bedroom apartment. I have children and pets, so please use eco-friendly cleaners if possible...'" 
                           className="w-full border border-slate-200 rounded-2xl p-6 focus:ring-2 focus:ring-[#2286BE] focus:border-transparent outline-none transition-all resize-none text-base font-medium bg-slate-50/50 placeholder:text-slate-400"
                         ></textarea>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                 <Card className="border-none shadow-xl shadow-slate-200/50 bg-white rounded-[2rem]">
                   <CardContent className="p-10">
                      ...
                      <p className="text-xs text-slate-400 mt-4 font-medium italic">Flexible dates often get more responses.</p>
                   </CardContent>
                 </Card>
                 ...
                 <Card className="border-none shadow-xl shadow-slate-200/50 bg-white rounded-[2rem]">
                   <CardContent className="p-10">
                      ...
                      <p className="text-xs text-slate-400 mt-4 font-medium italic">Providers can negotiate if your budget is flexible.</p>
                   </CardContent>
                 </Card>
              </div>
           </motion.div>
           ...
           <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-6">
              ...
              <div className="flex gap-4 w-full sm:w-auto">
                 ...
                 <Button 
                   ...
                   className="flex-[2] sm:flex-none bg-[#2286BE] hover:bg-[#059669] px-12 h-16 text-lg font-black rounded-2xl shadow-xl shadow-primary/20 active:scale-95 transition-all"
                 >
                    {isSubmitting ? (
