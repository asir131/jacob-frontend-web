...
const channels = [
  ...
  { icon: <Mail size={24} />, color: 'bg-emerald-500', title: 'Email', desc: 'support@locallyserve.com', sub: 'Response within 12 hrs' },
  { icon: <Phone size={24} />, color: 'bg-amber-500', title: 'Phone', desc: '+880 1700-000000', sub: 'Mon–Fri, 9AM–6PM' },
];
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <Input required placeholder="Karim Ahmed" className="h-14 rounded-[1.25rem] border-slate-200 focus-visible:ring-[#2286BE] font-medium" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-sm font-black text-slate-900 uppercase tracking-widest">Email</label>
                        <Input required type="email" placeholder="karim@email.com" className="h-14 rounded-[1.25rem] border-slate-200 focus-visible:ring-[#2286BE] font-medium" />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-black text-slate-900 uppercase tracking-widest">Subject</label>
                      <Input required placeholder="How can we help?" className="h-14 rounded-[1.25rem] border-slate-200 focus-visible:ring-[#2286BE] font-medium" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-sm font-black text-slate-900 uppercase tracking-widest">Message</label>
                      <textarea required rows={5} placeholder="Tell us more..." className="w-full border border-slate-200 rounded-[1.25rem] p-5 focus:ring-2 focus:ring-[#2286BE] focus:border-transparent outline-none transition-all resize-none font-medium text-slate-800" />
                    </div>
                    <Button type="submit" className="w-full h-16 bg-[#2286BE] hover:bg-[#059669] text-white font-black text-lg rounded-2xl shadow-xl shadow-[#2286BE]/20 active:scale-95 transition-all">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-[2rem] border border-slate-100 shadow-sm p-8">
              ...
              <div className="space-y-5">
                {[
                  { icon: <MapPin size={18} />, label: 'Address', value: 'Level 7, Banani Tower, Dhaka 1213, Bangladesh' },
                  { icon: <Mail size={18} />, label: 'Email', value: 'hello@locallyserve.com' },
                  { icon: <Phone size={18} />, label: 'Phone', value: '+880 1700-000000' },
                  { icon: <Clock size={18} />, label: 'Hours', value: 'Mon–Fri: 9:00 AM – 6:00 PM BST' },
                ].map(item => (
                  <div key={item.label} className="flex gap-4">
                    <div className="h-10 w-10 bg-primary-soft rounded-xl flex items-center justify-center text-[#2286BE] shrink-0">{item.icon}</div>
                    <div>
